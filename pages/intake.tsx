...
function SignaturePad({ onChange }: { onChange: (dataUrl: string) => void }) {
  ...
  return (
    <div>
      ...
      />
      <button type="button" onClick={clear} className="mt-2 underline text-sm">
        Borrar firma
      </button>
    </div>
  )
...
export default function Intake() {
    ...
    () => [
      {
        ...
        fields: [
          ...
          {
            id: 'q8_trabajo',
            label: 'Tu trabajo implica…',
            type: 'multi',
            options: [
              'Levantar o mover peso habitualmente',
              'Mantenerte de pie muchas horas',
              'Pasar sentado frente a un ordenador',
          ...
          {
            id: 'q8a_peso_frecuencia',
            label: '¿Cuánto peso aproximado sueles levantar y cuántas veces al día?',
            type: 'text',
            showIf: (v) => Array.isArray(v.q8_trabajo) && v.q8_trabajo.includes('Levantar o mover peso habitualmente'),
          },
          {
            id: 'q8b_ergonomia',
            label: '¿Usas ajustes ergonómicos (silla, reposapiés, pantalla ajustada)?',
            type: 'yesno',
            showIf: (v) => Array.isArray(v.q8_trabajo) && v.q8_trabajo.includes('Pasar sentado frente a un ordenador'),
          },
      ...
      {
        id: 'b3',
        title: 'Antecedentes del problema',
        fields: [
      ...
      {
        id: 'b4',
        title: 'Síntomas asociados (seguridad)',
        fields: [
      ...
      {
        id: 'b5',
        title: 'Limitaciones y vida diaria',
        fields: [
      ...
      {
        id: 'b7',
        title: 'Motivación y expectativas',
        fields: [
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        setValues((parsed.values || {}) as IntakeValues)
  ...
  const privacyText =
    'Autorizo a Clínica Podium al tratamiento de mis datos de salud con fines asistenciales y envío del acuse por email. He leído y acepto la Política de Privacidad (RGPD/LOPDGDD).'
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (!accepted || !signature) {
      alert('Debes aceptar y firmar el consentimiento')
    ...
    // Flags derivados
    const flags: string[] = []
    if (['q31_fiebre', 'q32_peso', 'q33_cancer', 'q36_esfinteres', 'q37_silla', 'q39_pecho'].some((k) => values[k] === 'Sí'))
      flags.push('RED_FLAG')
    if ((values.q19_irradia as string) === 'Se extiende') flags.push('RADIATING_PAIN')
    if ((values.q25_rigidez as string) === 'Sí' && Number(values.q25a_rigidez_min || 0) > 30) flags.push('STIFFNESS_>30MIN')
  ...
  const totalSteps = sections.length + 1 // +1 por consentimiento
  const currentStepDisplay = Math.min(clampedStep + 1, totalSteps)
    ...
    <form onSubmit={onSubmit} className="space-y-4 max-w-3xl mx-auto p-4">
      ...
      {clampedStep < sections.length ? (
        ...
        <section className="bg-white p-4 rounded-xl shadow space-y-2">
          ...
          <p className="text-sm text-gray-700">
            ...
            <br />
            Autorizo a Clínica Podium al tratamiento de mis datos de salud con fines asistenciales y envío del acuse por email. He leído y acepto la
      ...
      <div className="flex items-center justify-between gap-3">
        <button
          ...
          className={`px-4 py-3 rounded-xl border ${clampedStep === 0 ? 'opacity-40 cursor-not-allowed' : 'bg-white'}`}
        >
          ← Atrás
        </button>
        {clampedStep < sections.length ? (
